...
class ListTitleViewController: UIViewController {
    ...
    var coordinator : ListTitleViewControllerFlow?
    var viewModel: ListTitleViewModel = ListTitleViewModel()
    var tag = 0
...
extension ListTitleViewController {
    func initView() {
        shadowView()
        viewSetting()
...
extension ListTitleViewController {
    ...
    func jsonRequst(title: String? , image: String?) {
        viewModel.listTitleSet(title: title, image: image)
                .subscribe(
                    onSuccess: { response in
                        let idDelivery = IDDelivery.shared
                        idDelivery.listID = response.data.id
                        self.coordinator?.coordinateToTitleNextVC()
                    },
                    onError: { error in
                        debugPrint(error)
                    }
...
extension ListTitleViewController: UIPickerViewDelegate, UIPickerViewDataSource {
